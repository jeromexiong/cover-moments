...
typealias ElementTuple = (range: NSRange, element: JXElements, type: JXLabelType)
open class JXLabel: UILabel {
    ...
    // MARK: 公用 属性
    open var enabledTypes: [JXLabelType] = [.URL, .phone, .hashtag, .mention]
    ...
    open var customColor : [JXLabelType : UIColor] = [:] {
        didSet {updateTextStorage(updateString: false)}
    }
    open var customSelectColor : [JXLabelType : UIColor] = [:] {
        didSet {updateTextStorage(updateString: false)}
    }
    ...
    fileprivate lazy var elementDict = [JXLabelType: [ElementTuple]]()
    ...
    /// 自定义 点击事件
    open func handleCustomTap(_ type: JXLabelType, handler: @escaping LabelCallBack) {
...
fileprivate extension JXLabel {
    ...
    func creatElementTupleArr(type: JXLabelType, from textString: String, range: NSRange) -> [ElementTuple] {
        
        var elementTupleArr = [ElementTuple]()
        let nsstring = textString as NSString
        
        let matches = JXLabelRegex.getMatches(type: type, from: textString, range: range)
        for match in matches {
            ...
            elementTupleArr.append((range, JXElements.creat(with: type, text: word), type))
        }
